...
let handler = async (m, { conn }) => {
  try {
    ...
    if (!mime) {
      return m.reply('🖌️ Envía una *imagen* junto al comando para mejorarla.');
    }

    if (!/image\/(jpe?g|png)/.test(mime)) {
      return m.reply(`✖️ El formato *${mime}* no es compatible. Usa JPG o PNG.`);
    }
    ...
    if (!uploadedUrl) {
      return m.reply('✖️ No se pudo subir la imagen. Intenta nuevamente.');
    }
