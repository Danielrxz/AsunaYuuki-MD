...
const handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if (!data) return conn.reply(m.chat, '✖️ No se encontraron resultados.', m)
...
☁️ *Autor ›* ${author.nickname || author.unique_id || 'Desconocido'}
> ⏳ *Duración ›* ${duration || 'N/A'}
> 🌾 *Likes ›* ${(stats.likes || 0).toLocaleString()}
> ✉️ *Comentarios ›* ${(stats.comments || 0).toLocaleString()}
> 👀 *Vistas ›* ${(stats.views || stats.plays || 0).toLocaleString()}
> ⛄ *Compartidos ›* ${(stats.shares || 0).toLocaleString()}
> ☁️ *Audio ›* ${music.title ? music.title + " -" : 'Desconocido'} ${music.author ? music.author : ''}\n\n${dev}
    `.trim()
