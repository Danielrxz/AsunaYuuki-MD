const handler = async (m, { conn }) => {
  try {
   ...
   let who = texto.length > 0 ? texto[0] : (m.quoted ? await m.quoted.sender : false)
    if (!who) return m.reply('🌾 Menciona al usuario que deseas quitar de administrador.');
    ...
    if (who === groupMetadata.owner) {
      return m.reply('⭐ No puedes quitar al creador del grupo de administrador.');
    }

    if (who === conn.user.jid) {
      return m.reply('⭐ No puedes quitar al bot de administrador, Ni en tus sueños.');
    }
