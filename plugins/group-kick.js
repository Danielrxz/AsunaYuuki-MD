var handler = async (m, { conn, participants, usedPrefix, command }) => {
   ...
   let user = texto.length > 0 ? texto[0] : (m.quoted ? await m.quoted.sender : false)
    if (!user) {
        return conn.reply(m.chat, '☁️ Por favor, Etiqueta o menciona al usuario que quieras expulsar.', m);
    }
    ...
    if (user === m.sender) {
        return conn.reply(m.chat, '✖️ No puedes automencionarte, menciona a otro usuario.', m);
    }

    if (user === conn.user.jid) {
        return conn.reply(m.chat, '☁️ Yo como bot no puedo autoeliminarme del grupo.', m);
    }
    ...
    if (user === ownerBot) {
        return conn.reply(m.chat, '✖️ No puedo eliminar a mi creador', m);
    }

    const participant = groupInfo.participants.find(participant => participant.jid === user);

    if (!participant) {
        return conn.reply(m.chat, `✖️ El usuario *${globalThis.db.data.users[user].name}* ya no está en el grupo.`, m);
    }
