let handler = async (m, { conn, text, args }) => {
  ...
  try {
    ...
    if (accion === 'off') {
      if (estado) return m.reply('🌱 La *Bot* ya estaba *desactivada* en este grupo.');
      chat.bannedGrupo = true;
      return m.reply('🫟 Has *Desactivado* La *Bot* en este grupo.');
    }

    if (accion === 'on') {
      if (!estado) return m.reply('🌱 La *Bot* ya estaba *activada* en este grupo.');
    ...
    await m.reply('✖️ Ocurrió un error al intentar cambiar el estado del bot.');
  }
};
